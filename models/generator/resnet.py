...
from models.utils.layers import conv2d, instance_norm2d, deconv2d
...
def residule_block(x, num_outputs, kernel_size=3, stride=1, name='res'):
    ...
    y = instance_norm2d(conv2d(y, num_outputs, kernel_size, stride, padding='VALID', name=name + '_c1'), name + '_bn1')
    y = tf.nn.relu(y)
    y = tf.pad(y, [[0, 0], [padding, padding], [padding, padding], [0, 0]], 'REFLECT')
    y = instance_norm2d(conv2d(y, num_outputs, kernel_size, stride, padding='VALID', name=name + '_c2'), name + '_bn2')
    return y + x
...
def build_model(image, out_channels, filter_channels=64, reuse=False, name='resnet', **kwargs):
    with tf.variable_scope(name):
        ...
        c1 = tf.nn.relu(instance_norm2d(conv2d(c0, filter_channels, 7, 1, padding='VALID', name='g_e1_c'), 'g_e1_bn'))
        c2 = tf.nn.relu(instance_norm2d(conv2d(c1, filter_channels * 2, 3, 2, name='g_e2_c'), 'g_e2_bn'))
        c3 = tf.nn.relu(instance_norm2d(conv2d(c2, filter_channels * 4, 3, 2, name='g_e3_c'), 'g_e3_bn'))
        ...
        d1 = tf.nn.relu(instance_norm2d(deconv2d(r9, filter_channels * 2, 3, 2, name='g_d1_dc'), 'g_d1_bn'))
        d2 = tf.nn.relu(instance_norm2d(deconv2d(d1, filter_channels, 3, 2, name='g_d2_dc'), 'g_d2_bn'))
