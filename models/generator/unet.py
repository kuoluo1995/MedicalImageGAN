...
def build_model(image, out_channels, filter_channels=64, reuse=False, name='unet', is_training=False, **kwargs):
    ...
    with tf.variable_scope(name):
        ...
        d1 = deconv2d(tf.nn.relu(e7), filter_channels * 8, name='g_d1')
        d1 = tf.nn.dropout(d1, dropout_rate)
        d1 = tf.concat([instance_norm2d(d1, 'g_bn_d1'), e6], 3)

        d2 = deconv2d(tf.nn.relu(d1), filter_channels * 8, name='g_d2')
        d2 = tf.nn.dropout(d2, dropout_rate)
        d2 = tf.concat([instance_norm2d(d2, 'g_bn_d2'), e5], 3)

        d3 = deconv2d(tf.nn.relu(d2), filter_channels * 8, name='g_d3')
        d3 = tf.nn.dropout(d3, dropout_rate)
        d3 = tf.concat([instance_norm2d(d3, 'g_bn_d3'), e4], 3)

        d4 = deconv2d(tf.nn.relu(d3), filter_channels * 4, name='g_d4')
        d4 = tf.concat([instance_norm2d(d4, 'g_bn_d4'), e3], 3)

        d5 = deconv2d(tf.nn.relu(d4), filter_channels * 2, name='g_d5')
        d5 = tf.concat([instance_norm2d(d5, 'g_bn_d5'), e2], 3)

        d6 = deconv2d(tf.nn.relu(d5), filter_channels, name='g_d6')
        d6 = tf.concat([instance_norm2d(d6, 'g_bn_d6'), e1], 3)

        # d7 = deconv2d(tf.nn.relu(d6), filter_channels, name='g_d7')
        # d7 = tf.concat([instance_norm2d(d7, 'g_bn_d7'), e1], 3)

        d8 = deconv2d(tf.nn.relu(d6), out_channels, name='g_d7')
