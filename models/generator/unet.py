...
def build_model(image, out_channels, filter_channels=64, reuse=False, name='unet', is_training=False, **kwargs):
    ...
    with tf.variable_scope(name):
        ...
        e1 = instance_norm(conv2d(image, filter_channels, name='g_e1_conv'))
        e2 = instance_norm(conv2d(leaky_relu(e1), filter_channels * 2, name='g_e2_conv'), 'g_bn_e2')
        e3 = instance_norm(conv2d(leaky_relu(e2), filter_channels * 4, name='g_e3_conv'), 'g_bn_e3')
        e4 = instance_norm(conv2d(leaky_relu(e3), filter_channels * 8, name='g_e4_conv'), 'g_bn_e4')
        e5 = instance_norm(conv2d(leaky_relu(e4), filter_channels * 8, name='g_e5_conv'), 'g_bn_e5')
        e6 = instance_norm(conv2d(leaky_relu(e5), filter_channels * 8, name='g_e6_conv'), 'g_bn_e6')
        e7 = instance_norm(conv2d(leaky_relu(e6), filter_channels * 8, name='g_e7_conv'), 'g_bn_e7')
        e8 = instance_norm(conv2d(leaky_relu(e7), filter_channels * 8, name='g_e8_conv'), 'g_bn_e8')
