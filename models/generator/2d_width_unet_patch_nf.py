...
def build_model(batch_input, out_channels, filter_channels=64, reuse=False, name='unet', is_training=False, **kwargs):
    ...
    with tf.variable_scope(name):
        ...
        e8 = instance_norm2d(conv2d(leaky_relu(e7), filter_channels * 128, name='g_conv_e8'), 'g_in_e8')

        d8 = instance_norm2d(deconv2d(tf.nn.relu(e8), filter_channels * 64, name='g_conv_d8'), 'g_in_d8')
        d8 = tf.nn.dropout(d8, keep_prob=dropout_rate)
        d8 = tf.concat([d8, e7], 3)

        d7 = instance_norm2d(deconv2d(tf.nn.relu(d8), filter_channels * 32, name='g_conv_d7'), 'g_in_d7')
        d7 = tf.nn.dropout(d7, keep_prob=dropout_rate)
        d7 = tf.concat([d7, e6], 3)

        d6 = instance_norm2d(deconv2d(tf.nn.relu(d7), filter_channels * 16, name='g_conv_d6'), 'g_in_d6')
