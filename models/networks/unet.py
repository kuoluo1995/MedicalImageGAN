...
def unet(image, G_channels, out_channels, is_training, reuse=False, name="generator", **kwargs):
    ...
    with tf.variable_scope(name):
        ...
        # e8 = instance_norm(conv2d(leaky_relu(e7), G_channels * 8, name='g_e8_conv'), 'g_bn_e8')
        ...
        d4 = deconv2d(tf.nn.relu(d3), G_channels * 4, name='g_d4')
        d4 = tf.concat([instance_norm(d4, 'g_bn_d4'), e3], 3)

        d5 = deconv2d(tf.nn.relu(d4), G_channels * 2, name='g_d5')
        d5 = tf.concat([instance_norm(d5, 'g_bn_d5'), e2], 3)

        d6 = deconv2d(tf.nn.relu(d5), G_channels, name='g_d6')
        d6 = tf.concat([instance_norm(d6, 'g_bn_d6'), e1], 3)

        # d7 = deconv2d(tf.nn.relu(d6), G_channels, name='g_d7')
