...
class Pix2PixGANRESEDGE(BaseGanModel):
    ...
    def build_model(self):
        ...
        self.res = self.real_b - self.real_a
        self.fake_res = self.generator(self.real_a, is_training=True, name='generator_a2b')

        fake_res_edges = tf.image.sobel_edges(self.fake_res)
        res_edges = tf.image.sobel_edges(self.res)

        fake_b = self.real_a + self.fake_res
        fake_ab = tf.concat([self.real_a, fake_b], 3)
        fake_logit_b = self.discriminator(fake_ab, name='discriminator_b')
        self.g_loss = self.loss_fn(fake_logit_b, tf.ones_like(fake_logit_b)) + \
                      self.i_lambda * l1_loss(self.fake_res, self.res) + \
                      self.e_lambda * l1_loss(fake_res_edges, res_edges)
        ...
        self.fake_res_sample = tf.placeholder(tf.float32, [None, data_shape[0], data_shape[1], self.out_channels],
                                              name='res')
        fake_b = self.real_a + self.fake_res_sample
        fake_ab = tf.concat([self.real_a, fake_b], 3)
        ...
        self.d_loss_b = d_loss_real_b + d_loss_fake_b + self.e_lambda * l1_loss(fake_res_edges, res_edges)
    ...
    def test(self):
        ...
        for step in range(test_size + 1):
            ...
            nii_b.append(batch_b[0, :, :, self.out_channels // 2])
            fake_nii_b.append(batch_a[0, :, :, self.out_channels // 2] + fake_res[0, :, :, self.out_channels // 2])
